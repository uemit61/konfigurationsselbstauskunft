<?php
    ...
    class Start_Model
    {
        ...
        private $glorixx_customer_id = null;
        ...
        function get_infos_from_database()
        {
            try
            {
                $con = $this->pdo->connect_to_database();
                $query = "SELECT Glorixx_Customer_Id,Logo FROM grorixx_customer WHERE customer_id = :customer_id";
                ...
                session_start();
                    $_SESSION['Glorixx_Customer_ID'] = $result['Glorixx_Customer_ID'];
                    $this->glorixx_customer_id = $result['Glorixx_Customer_ID'];
...
?>
                ...
                <script> console.log('Das Token <?php echo $this->token; ?> ist gültig. Passwortseite wird geöffnet.');</script>
<?php 
        ...
        function check_password($user,$password)
        {
            ...
            if(password_verify($pepperPass,'$argon2id$v=19$m=65536,t=4,p=1$'.$test))
            {
                ...
                {
                    $con = $this->pdo->connect_to_db();
                    $query = "  SELECT Json_Data, Logo 
                                FROM `json_cache`, `glorixx_customer` 
                                WHERE Customer_ID = :Customer_ID OR Glorixx_Customer_ID = :Glorixx_Customer_ID";
    
                    $stmt = $con->prepare($query);
                    $stmt->bindValue(':Customer_ID',$this->customer_id);
                    $stmt->bindValue(':Glorixx_Customer_ID',$this->glorixx_customer_id);
                    $stmt->execute();
                    $con =null;
                    ...
                    echo "Zwichenspeicher wurde geladen.";
                }
                catch (PDOException $e)
                {
